...
function DetailIcon(props) {
    ...
    const renderUp = () => {
        const prev = _.find(props.detailArr, { number: props.detailArr[_.find(props.detailArr, { id: props.contactId }).number].number - 1 })
    ...
    const renderDown = () => {
        const next = _.find(props.detailArr, { number: props.detailArr[_.find(props.detailArr, { id: props.contactId }).number].number + 1 })
        ...
        <div style={{ textAlign: 'right' }}>
            ...
            <Dropdown icon='bars' pointing='right' className='articleIcon'>
                <Dropdown.Menu>
                    ...
                    />
                    <Dropdown.Item
                        onClick={() => { dispatch(deleteDetail(props.contactId)) }}
                        content={t("Delete")}
                        icon='trash'
                    />
                    <Dropdown.Divider />
